...
% Not auxotrophic for anything, exports everything.
Ec5 = iAF1260;
%Ec5 = changeRxnBounds(Ec5, [argH; lysA; metA; ilvE], 0, 'b');
%Ec5 = changeRxnBounds(Ec5, [argO; lysO; yjeH; yddG], 0, 'b');
...
EcCom.lb(EcCom.indCom.EXsp) = repmat(lbEx(id), 1, 5); % assign organism-specific uptake bounds
...
% Ec5
EcCom = changeRxnBounds(EcCom, {'Ec5IEX_arg__L[u]tr'; 'Ec5IEX_lys__L[u]tr'; 'Ec5IEX_met__L[u]tr'; 'Ec5IEX_phe__L[u]tr'}, 0, 'l');
EcCom = changeRxnBounds(EcCom, {}, -exRate, 'l');
% allow production of anything for each member
EcCom.ub(EcCom.indCom.EXsp(:)) = 1000;

% Now make 5 E.Coli model to be used in Python code.
[I,J] = size(EcCom.S);
S = EcCom.S;
lb = EcCom.lb;
ub = EcCom.ub;

cd('/Users/mitchellperry/Dropbox/Microbiome (cleaned up)/FiveEColiModelforPython')
...
Ec5_reactions_idx = find(EcCom.indCom.rxnSps == 5);
Ec5_metabolites_idx = find(EcCom.indCom.metSps == 5);
Ec5_reactions = EcCom.rxns(Ec5_reactions_idx);
Ec5_biomass_idx = find(strcmp(Ec5_reactions, EcCom.infoCom.spBm(5)) == 1);

save('Ec5_reactions_idx.mat', 'Ec5_reactions_idx');
save('Ec5_metabolites_idx.mat', 'Ec5_metabolites_idx');
save('Ec5_biomass_idx.mat', 'Ec5_biomass_idx');
save('Ec5_reactions.mat', 'Ec5_reactions');